...
bai_questions = [
    ...
    "8. عدم الثبات أو التمايل","9. خوف من فقدان السيطرة","10. رعشة في اليدين","11. ارتجاف عام في الجسم",
    "12. خوف من الموت","13. العصبية أو التوتر","14. الشعور بالاختناق","15. ارتعاش في الجسم",
...
phq9_questions = [
    ...
    "3. صعوبة النوم أو النوم المفرط","4. الشعور بالتعب أو انخفاض الطاقة","5. فقدان الشهية أو الإفراط في الأكل",
    "6. الشعور بسوء تقدير الذات أو الشعور بالفشل","7. صعوبة التركيز على الأمور اليومية",
    "8. البطء الحركي أو التهيج","9. أفكار متكررة عن الموت أو إيذاء النفس"
]
...
bai_answers = {q: st.radio(q, options=list(bai_choices.keys()), key=f"bai_{i}") for i, q in enumerate(bai_questions)}
phq9_answers = {q: st.radio(q, options=list(phq9_choices.keys()), key=f"phq9_{i}") for i, q in enumerate(phq9_questions)}
isi_answers = {q: st.radio(q, options=list(isi_choices.keys()), key=f"isi_{i}") for i, q in enumerate(isi_questions)}
...
if st.button("Submit"):
    ...
    st.success("PDF and CSV are ready to download!")
