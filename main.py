...
# -----------------------
bai_questions = [
    "1. تنميل أو وخز","2. الشعور بالحرارة","3. ارتجاف في الساقين",
    "4. عدم القدرة على الاسترخاء","5. الخوف من حدوث أسوأ الأشياء",
    "6. دوخة أو دوار أو فقدان التوازن","7. خفقان أو زيادة ضربات القلب",
    "8. عدم الثبات أو التمايل","9. خوف من فقدان السيطرة",
    "10. رعشة في اليدين","11. ارتجاف عام في الجسم","12. خوف من الموت",
    "13. العصبية أو التوتر","14. الشعور بالاختناق","15. ارتعاش في الجسم",
    "16. الشعور بالخوف","17. اضطراب أو انزعاج في المعدة","18. الشعور بالإغماء أو فقدان الوعي",
...
with st.form("patient_form"):
    ...
    bai_score = sum(int(st.radio(q, options=["0","1","2","3"], format_func=lambda x: ["أبدًا","قليلاً","نصف الأيام","تقريبًا كل يوم"][int(x)])) for q in bai_questions)
    phq9_score = sum(int(st.radio(q, options=["0","1","2","3"], format_func=lambda x: ["أبدًا","عدة أيام","أكثر من نصف الأيام","تقريبًا كل يوم"][int(x)])) for q in phq9_questions)
    isi_score = sum(int(st.radio(q, options=["0","1","2","3","4"], format_func=lambda x: ["لا أبدًا","قليل","متوسط","كثير","شديد جدًا"][int(x)])) for q in isi_questions)
    ...
    if submitted:
        ...
        writer.writerow(["Patient Name","Age","Tumor Type","BAI Score","BAI Level","PHQ-9 Score","PHQ-9 Level","ISI Score","ISI Level","Date"])
        writer.writerow([patient_name, age, tumor_type, bai_score, bai_result, phq9_score, phq9_result, isi_score, isi_result, datetime.now().strftime("%Y-%m-%d %H:%M:%S")])
        csv_buffer.seek(0)
        
        # -----------------------
        # أزرار التحميل
        # -----------------------
        st.download_button("Download PDF", pdf_buffer, file_name=f"{patient_name}_report.pdf", mime="application/pdf")
        st.download_button("Download CSV", csv_buffer, file_name="patients_data.csv", mime="text/csv")
        st.success("Results calculated! PDF and CSV ready for download.")
