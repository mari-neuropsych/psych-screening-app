...
with st.form("patient_form"):
    ...
    for q in bai_questions:
        bai_score += int(st.radio(q, options=["0","1","2","3"], format_func=lambda x: ["أبدًا","قليلاً","نصف الأيام","تقريبًا كل يوم"][int(x)]))
    
    phq9_score = 0
    for q in phq9_questions:
        phq9_score += int(st.radio(q, options=["0","1","2","3"], format_func=lambda x: ["أبدًا","عدة أيام","أكثر من نصف الأيام","تقريبًا كل يوم"][int(x)]))
    
    isi_score = 0
    for q in isi_questions:
        isi_score += int(st.radio(q, options=["0","1","2","3","4"], format_func=lambda x: ["لا أبدًا","قليل","متوسط","كثير","شديد جدًا"][int(x)]))
